...
if __name__ == "__main__":
    # Example usage
    try:
        results = search_web_firecrawl("Analyze the impact of AI on healthcare in the last 5 years", 1)
        print("\n=== Search Results ===\n")
        for result in results:
            print(f"Title: {result['title']}")
            print(f"URL: {result['url']}")
            print(f"Description: {result['description']}")
            print(f"Content: {result['data'][:200]}...")  # Print first 200 chars of content
            print("\n---\n")
    except Exception as e:
        print(f"Error: {e}") 