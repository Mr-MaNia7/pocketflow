...
class ResearchLogger:
    ...
    def log_step(self, node_name: str, action: str, message: str, data: Dict[str, Any] = None):
        ...
        console.print(Panel(table, title=f"Step {len(self.steps)}", border_style="blue"))
    ...
    def log_completion(self, summary: Dict[str, Any]):
        ...
        summary.update({
            ...
            "successful_steps": len([s for s in self.steps if s["action"] != "ERROR"])
        })
        ...
        console.print(Panel(table, title="Research Process Summary", border_style="green"))
        
        # Save the log to a file
        os.makedirs("logs", exist_ok=True)
        log_file = os.path.join("logs", f"research_log_{self.start_time.strftime('%Y%m%d_%H%M%S')}.json")
